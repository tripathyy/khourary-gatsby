...
exports.createPages = async ({ actions, graphql, reporter }) => {
  ...
  const queryResult = await graphql(`
    {
      ...
      serviceQuery: allWpService(sort: { fields: date, order: DESC }) {
        edges {
          node {
            databaseId
            uri
  ...
  // Generate single post pages
  const services = queryResult.data.serviceQuery.edges
  services.forEach(service => {
    createPage({
      ...
      context: {
        // Data passed to context is available
        // in page queries as GraphQL variables.
        databaseId: service.node.databaseId,
        nextId: service.next ? service.next.databaseId : null,
        prevId: service.previous ? service.previous.databaseId : null,
      },
    })
  })
}